...
var desc = _(""
	+ "<p>This service handles parsing the eTactica interval stream data, and writing to your Sustainable Exergy Database"
	+ '<img src="/resources/images/logo_sustainable-exergy.png" width="340" height="151" style="vertical-align: middle"/>'
	+ "<h4>About Sustainable Exergy</h4>"
	+ "From their website:"
	+ "<pre>"
	+ "We believe that digitalization will revolutionize environment worldwide and improve our lives with intelligent devices. "
	+ "With <em>SE Monitoring</em> we aim to make the greatest possible impact in addressing the environmental issues associated with use of "
	+ "resources like energy, water, gas etc. For more information please visit our web site: <a href='http://www.sustainable-exergy.de'>www.sustainable-exergy.de</a>"
	+ "</pre>"
	+ '<a href="http://www.sustainable-exergy.de/" target="_blank">Visit their site for more information</a>'
);
...
return L.view.extend({
	render: function() {
		...
		m = new form.Map('output-sustainable-exergy', _('Output Sustainable Exergy'), desc);

		s = m.section(form.TypedSection, 'instance', _('Options'));
		s.addremove = false;
		s.anonymous = true;
		...
		var load_q = function(sid, qtype) {
			var custom_fn = "/etc/output-sustainable-exergy/custom." + sid + "." + qtype + ".query";
			var custom = fs.trimmed(custom_fn);
			var def = fs.trimmed("/usr/share/output-sustainable-exergy/default." + qtype + ".query");
		...
		o.rows = 20;
		o.cfgvalue = function(section_id) {
			var custom_fn = "/etc/output-sustainable-exergy/custom." + section_id + ".schema";
			var custom = fs.trimmed(custom_fn);
			var def = fs.trimmed("/usr/share/output-sustainable-exergy/schema." + driver.cfgvalue(section_id) + ".sql");
			return Promise.all([custom, def]).then(function(values) {
				// priority list
		...
		o.placeholder = "apps.output-sustainable-exergy.<instanceid>"
		o.optional = true
